...
def plot_experiment_results(only_plot_aggregators : Set[int], only_plot_seed : Set[int], image_format : str):
    results_path = "fairness_benchmark_2/results"
    ...
    for seed_directory in seed_list:
        ...
        fig_deo, ax_deo = plt.subplots(figsize=(6, 6)) # plot for DEO1
        fig_d, ax_d = plt.subplots(figsize=(6, 6)) # plot for norm of d
        ...
        for data_filename in aggregator_list:
            ...
            ax_deo.plot(n_iteration, deo, color = colours[aggregator_name],label = aggregator_name, alpha = 0.5)
            ax_d.plot(n_iteration, track_norm_d, color = colours[aggregator_name],label = aggregator_name, alpha = 0.5)
            ...
            ax_combined[0,1].plot(n_iteration, deo, color = colours[aggregator_name],label = aggregator_name, alpha = 0.5)
            ax_combined[1,0].plot(n_iteration, track_norm_d, color = colours[aggregator_name],label = aggregator_name, alpha = 0.5)
            ...
            ax_combined2[row_number,1].plot(n_iteration, deo, color = colours[aggregator_name],label = aggregator_name, alpha = 0.5)
            ax_combined2[row_number,2].plot(n_iteration, track_norm_d, color = colours[aggregator_name],label = aggregator_name, alpha = 0.5)
            ...
            if aggregator_name == "MGDA": # MGDA is the only aggregator that is plotted on both rows
                ...
                ax_combined2[1,1].plot(n_iteration, deo, color = colours[aggregator_name],label = aggregator_name, alpha = 0.5)
                ax_combined2[1,2].plot(n_iteration, track_norm_d, color = colours[aggregator_name],label = aggregator_name, alpha = 0.5)
        ...
        ax_deo.set_title("DEO1", fontsize = 18)
        ...
        ax_deo.legend(loc='upper right', prop={'size': 14})
        ...
        fig_deo.savefig(os.path.join(plot_path, f"DEO.{image_format}"), dpi = 300)
        ...
        ax_combined[0,1].set_title("DEO", fontsize = 18)
        ...
        ax_combined2[0,1].set_title("DEO", fontsize = 18)
        ...
        ax_combined2[1,1].set_title("DEO", fontsize = 18)
