...
def iterate(problem, aggregator, seed, epoch, lr, eps) -> Tuple[np.array]:
    ...
    for iter_count in range(epoch):
        ...
        if iter_count+1 % 1000 == 0:
            ...
            print(f"Epoch = {iter_count+1}/{epoch}, norm_d = {norm_d}, norm_alpha = {norm_alpha}, norm_d_mgda = {norm_d_mgda}")
    ...
    return (x_trajectory, y_trajectory, track_d, track_alpha, track_measure_to_PS)

def run_experiment(problems : List, aggregators : List, seeds : List[int], lr : float, epoch : int, eps : float):
    ...
    with open(f"{results_path}/experiment_details.txt", "w") as f: 
        ...
        print(f"Number of epoch {epoch}", file = f)
    ...
    for problem in problems:
        ...
        for seed in seeds:
            ...
            for aggregator in aggregators:
                ...
                print(f"Seed : {seed}")
                x_trajectory, y_trajectory, track_norm_d, track_alpha, track_measure_to_PS = iterate(problem, aggregator, seed, epoch, lr, eps)
