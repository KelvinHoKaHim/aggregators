...
def train(model, aggregator, mgda, seed, eps = 1e-3, learning_rate = 0.001, num_epochs = 65000):
    ...
    for epoch in range(num_epochs): # start training
        ...
        grad3_flat = torch.cat([g.view(-1) for g in grad2])  # Flatten into a vector
        model.zero_grad()
    ...
    data = {
        ...
        "third loss function" : track_loss3,
        "norm of d" : track_d,
    ...
    return (track_loss1, track_loss2, track_loss3, track_d, track_d_MGDA)
...
if __name__ == "__main__":
    ...
    for seed in [64, 128]: # run with different random seeds
        for aggregator in aggregators:
            ...
            for title, curve in zip(["First loss function", "Second loss functions", "Third loss functions", "Norm of d" , "Measure of Pareto Stationarity"], [track_loss1, track_loss2, track_loss3, track_d, track_d_MGDA]):
            ...
            plt.figure(figsize=(12, 18))  # Create a new figure for the combined plot
            ...
            plt.subplot(3,2,5)
            plt.plot(iterations, track_loss3)
            plt.title("Third loss functions")
            plt.xlabel("Number of iterations")
            ...
            np.savetxt(txt_path, [success_rate, fairness1, fairness2])
